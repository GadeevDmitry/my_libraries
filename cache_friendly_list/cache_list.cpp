                                                                        ...
                                                                        const size_t ind_next)
{
    ...
    if ($size + 1 == $capacity) { if (!list_resize(lst)) return false; }
    ...
    return list_busy_node_ctor(lst, data, ind_cur, ind_prev, ind_next);
}
...
$   if ($el_dtor == nullptr) { log_free($fictional); return; }
...
{
$i
    if (!list_header_dump(lst)) return;
